...
The feature is something I‘m going ~~to implement soon~~. <strong>It‘s already implemented!</strong> Check out the [front page]({{ site.url }}). It‘s about showing some information from a third-party service. Obviously, I can‘t just directly call the service from the front-end since I have to include private keys in the request. What I need instead is to expose my own endpoint on a server and call the service from there.
